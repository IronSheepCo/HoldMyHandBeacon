...
ble_os_t m_debug_service;                                                           /*debug service*/

#define IS_SRVC_CHANGED_CHARACT_PRESENT 1                                           /**< Include or not the service_changed characteristic. if not enabled, the server's database cannot be changed for the lifetime of the device*/

#define DEVICE_NAME                     "HMHBeacon" "" BEACON_NO                                /**< Name of device. Will be included in the advertising data. */

int     APP_ADV_INTERVAL = 160;                                                    /**< The advertising interval (in units of 0.625 ms) Using a var so debug service can change it. */
uint8_t  APP_TX_LEVEL = 0;                                                           /**< The antena strength. This is measured in db. Valid values: -40, -30, -20, -16, -12, -8, -4, 0, 4, in hex: D8, E2, EC, F0, F4, F8, FC, 0, 04*/
int APP_POS_X = 0;
int APP_POS_Y = 0;
int APP_ALT = 0;
...
static void gap_params_init(void)
{
    ...
    // Store the device name and security mode in the SoftDevice. Our name is defined to "HMHBeacon" in the beginning of this file
...
{
    debug_service_init( &m_debug_service );
}
...
static void advertising_init(void)
{
    ...
    ble_uuid_t          adv_uuids[] = {{0x2346, BLE_UUID_TYPE_BLE}};    // Random example User Unique Identifier
    ...
    memcpy( data, &APP_POS_X, sizeof(int) );
    memcpy( data+sizeof(int), &APP_POS_Y, sizeof(int));
    memcpy( data+sizeof(int)+sizeof(int), &APP_TX_AT_1M, sizeof(int8_t));
    ...
    advdata.flags                   = flags;   
    advdata.p_tx_power_level        = &APP_TX_AT_1M;                                  //
    srdata.uuids_complete.uuid_cnt = sizeof(adv_uuids) / sizeof(adv_uuids[0]); //
    srdata.uuids_complete.p_uuids  = adv_uuids;                                //

    srdata.p_manuf_specific_data = &manuf_data;
    srdata.p_tx_power_level         = &APP_TX_AT_1M;
    ...
    SEGGER_RTT_printf(0,"advertising interval: %d\n", APP_ADV_INTERVAL);
...
static void advertising_start(void)
{
    ...
    SEGGER_RTT_printf(0,"advertising interval: %d", APP_ADV_INTERVAL);
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    debug_service_on_ble_evt(&m_debug_service, p_ble_evt);
}
